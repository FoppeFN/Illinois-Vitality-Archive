...
class City(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            GinIndex(
                fields=["city_name"], name="city_name_trgm", opclasses=["gin_trgm_ops"]
            )
        ]
...
class Person(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            GinIndex(
                fields=["first_name"],
                name="person_first_name_trgm",
                opclasses=["gin_trgm_ops"],
            ),
            GinIndex(
                fields=["last_name"],
                name="person_last_name_trgm",
                opclasses=["gin_trgm_ops"],
            ),
            GinIndex(
                fields=["middle_name"],
                name="person_middle_name_trgm",
                opclasses=["gin_trgm_ops"],
            ),
        ]
    ...
    first_name = models.CharField(
        db_index=True, max_length=100, blank=True, default="Unknown"
    )
    middle_name = models.CharField(
        db_index=True, max_length=100, blank=True, default=""
    )

    # sex
    sex = models.CharField(
        db_index=True, max_length=1, choices=Sex.choices, blank=True, null=True
    )
    # =================================================
    ...
    def siblings(self, sibling_sex=None):
        # return a queryset i.e. matching siblings when parents exists
        if not self.mother and not self.father:
            return Person.objects.none()
        #  when no parents available
    ...
    def spouses(self):
        marriages = Marriage.objects.filter(
            models.Q(spouse1=self) | models.Q(spouse2=self)
        )
        spouses = []
...
class Birth(models.Model):
    ...
    birth_record_image = models.ImageField(
        upload_to="birth_records/", blank=True, null=True
    )
...
class Death(models.Model):
    ...
    death_record_image = models.ImageField(
        upload_to="death_records/", blank=True, null=True
    )
...
class Marriage(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["spouse1", "spouse2", "marriage_date"], name="unique_marriages"
            )
        ]

    spouse1 = models.ForeignKey(
        Person, on_delete=models.CASCADE, related_name="marriages_as_spouse1"
    )

    spouse2 = models.ForeignKey(
        Person, on_delete=models.CASCADE, related_name="marriages_as_spouse2"
    )
    ...
    marriage_record_image = models.ImageField(
        upload_to="marriage_records/", blank=True, null=True
    )

    def spouse(self, person):
        if person == self.spouse1:
            return self.spouse2
        if person == self.spouse2:
            return self.spouse1
        return None
...
class Comment(models.Model):
    ...
    commenter_email = models.CharField(
        db_index=True, max_length=100, blank=True, null=True
    )
